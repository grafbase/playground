 ...
 */
const isOneOfType = (name: string, type: string) =>
  name === 'by' && type.endsWith('ByInput')
...
const mapArg = (schema: GraphQLSchema, a: any, maxLevel = 1): MappedArgs => {
  ...
  const typeName = type
    ?.toString?.()
    ?.replace(/\[|\]|!/gi, '') as PlaceholderTypes // remove "!", "[" and "]"
  ...
  const nodes =
    maxLevel > 0
      ? edges.map((n) =>
          mapArg(
            schema,
            { ...n, isByInput: isOneOfType(name, typeName) },
            maxLevel - 1
          )
        )
      : []
  const required =
    maxLevel > 0
      ? a.type.toString().endsWith('!') || !!isByInput || !placeholder
      : false
...
const getExampleMutation = async (schema: GraphQLSchema) => {
  ...
  const createMutation = filteredMutations.find((f) =>
  ...
  if (createMutation) {
    ...
    const mappedPayloadFields = Object.values(payloadFields).map((a) =>
      mapArg(schema, a, 3)
    )

    const stringifiedArgs = mappedArgs.reduce(stringifyMap(true), '')
    const stringifiedPayload = mappedPayloadFields.reduce(
      stringifyMap(false),
      ''
    )
  ...
  const deleteMutation = filteredMutations.find((f) =>
...
const getExamplePagination = async (schema: GraphQLSchema) => {
  ...
  const collectionQuery = filteredCollections.find((f) =>
    f.name.includes('Collection')
  )
